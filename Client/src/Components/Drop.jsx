...
import { Upload, FileText, XCircle, Loader, AlertTriangle } from "lucide-react";
  ...
  const handleExtractText = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `API Error: ${errorBody.error?.message || response.statusText}`
        );
      }
      ...
      if (result.candidates?.[0]?.finishReason === "SAFETY") {
        throw new Error(
          "Content was blocked due to safety concerns. Please try a different file."
        );
      }
      ...
      if (text) {
        ...
        throw new Error(
          "No text could be extracted from the file. The API response might be empty."
        );
      }
    } catch (err) {
        ...
        <div className="w-full max-w-3xl rounded-2xl shadow-2xl p-6 md:p-8 transform transition-all duration-500">
          <div className="text-center mb-8">
            ...
            <p className="mt-4 text-lg text-gray-300 max-w-2xl mx-auto">
              Upload PDFs, images, or scanned files and let our AI-powered OCR
              system instantly extract text with high accuracy. Transform
              unstructured data into searchable, editable, and organized
          ...
          {!file && (
            <div
              ...
              className={`flex flex-col items-center justify-center w-full border-2 border-dashed rounded-2xl p-12 cursor-pointer transition-all duration-300 ease-in-out ${
                isDragActive
                  ? "border-indigo-500 bg-gray-800 scale-105"
                  : "border-gray-600 bg-gray-800/50 hover:border-indigo-500"
              }`}
            >
              <input {...getInputProps()} />
          ...
          {file && (
            <div className="animate-fade-in-up mt-4 flex items-center justify-between gap-3 p-4 border border-gray-700 rounded-xl bg-gray-800 shadow-sm">
              ...
              <button
                ...
                className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-900/30 rounded-full transition-colors duration-200"
              >
            ...
            <div className="animate-fade-in-up mt-8">
              ...
              <div className="bg-[#1e1e1e] rounded-lg shadow-2xl overflow-hidden border border-gray-700/10">
                <div className="bg-gray-700 flex items-center p-2.5">
                  ...
                  <p className="text-center text-sm font-medium text-gray-300 flex-grow -ml-10">
                    Edit your Data
                  </p>
                </div>
              ...
              <div className="text-center mt-4">
                ...
                {message && <p className="mt-2 text-gray-200">{message}</p>}
              </div>
            </div>
          )}
        </div>
